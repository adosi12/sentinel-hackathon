      ...
      <div className="p-4 font-mono text-xs overflow-x-auto">
        {activeTab === 'logs' && (
          <div className="flex flex-col gap-1 text-white/70">
            <div className="flex gap-4 hover:bg-white/5 p-1 rounded"><span className="text-white/30 w-8">1</span><span className="text-white/40">10:41:01</span><span className="text-blue-400 w-12">INFO</span><span>com.bank.payments.GatewayFilter - Request received TXN_98765 from 10.12.4.5</span></div>
            <div className="flex gap-4 hover:bg-white/5 p-1 rounded"><span className="text-white/30 w-8">2</span><span className="text-white/40">10:41:03</span><span className="text-red-400 w-12">ERROR</span><span>com.bank.payments.TransactionSerializer - NoSuchMethodError: com.fasterxml.jackson.databind.ObjectMapper.readTree(java/lang/String;)</span></div>
            <div className="flex gap-4 hover:bg-white/5 p-1 rounded"><span className="text-white/30 w-8">3</span><span className="text-white/40">10:41:04</span><span className="text-orange-400 w-12">WARN</span><span>com.bank.payments.GatewayFilter - Upstream connection reset after 5000ms timeout</span></div>
            <div className="flex gap-4 hover:bg-white/5 p-1 rounded bg-red-500/10"><span className="text-white/30 w-8">4</span><span className="text-white/40">10:41:05</span><span className="text-red-400 font-bold w-12">ERROR</span><span className="text-white/90">com.bank.payments.PaymentServlet - HTTP 504 Gateway Timeout returned to client TXN_98765</span></div>
            <div className="flex gap-4 hover:bg-white/5 p-1 rounded"><span className="text-white/30 w-8">5</span><span className="text-white/40">10:42:00</span><span className="text-purple-400 w-12">FATAL</span><span>com.bank.payments.HealthCheck - 3 consecutive 504 responses - marking service DEGRADED</span></div>
          </div>
        )}
        {activeTab === 'metrics' && (
